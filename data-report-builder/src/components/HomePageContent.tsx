...
export const HOME_ENTITIES = Array.from(new Set(
...
// Labels for the overview widgets
...
/**
 * Home page content - original layout
 */
export function HomePageContent() {
  ...
  const currentRange = useMemo(() => {
    ...
    return preset ? preset.getValue() : rangePresets[5].getValue();
  }, [selectedRange]);

  // Compute Gross Volume for the Today section
  const grossVolumeData = useMemo(() => {
    ...
    const hourlyData = Array.from({ length: 24 }, (_, i) => {
      ...
      let multiplier = 0.3;
      if (i >= 6 && i < 9) multiplier = 0.6;
      else if (i >= 9 && i < 12) multiplier = 1.2;
      else if (i >= 12 && i < 14) multiplier = 0.9;
      else if (i >= 14 && i < 18) multiplier = 1.1;
      else if (i >= 18 && i < 21) multiplier = 0.8;
      else if (i >= 21) multiplier = 0.4;
